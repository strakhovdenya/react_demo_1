...
const Timeline: React.FC<TimelineProps> = ({ events, onEditEvent }) => {
  ...
  const timeSlots: TimelineSlotType[] = Array.from({ length: 24 * 4 }, (_, i) => {
  ...
  const isTimeInBusyInterval = (time: string) => {
    return busyIntervals.some(interval => {
      ...
      const startInMinutes = parseInt(interval.start.split(':')[0]) * 60 + parseInt(interval.start.split(':')[1]);
      const endInMinutes = parseInt(interval.end.split(':')[0]) * 60 + parseInt(interval.end.split(':')[1]);
      ...
      >
        <Box sx={{ position: 'relative' }}>
          ...
          {busyIntervals.map((interval, index) => {
            const { top, height } = getIntervalPosition(timeSlots, interval.start, interval.end);
            return (
